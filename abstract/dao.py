class DAO:
    ...
    def _asdict(obj):
        if isinstance(obj, dict):
            return {key: DAO._asdict(value) for key, value in obj.items() if value is not None}
        elif isinstance(obj, list):
            return [DAO._asdict(element) for element in obj if element is not None]
        elif isinstance(obj, tuple):
            return tuple([DAO._asdict(element) for element in obj if element is not None])
